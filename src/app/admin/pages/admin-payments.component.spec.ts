...
describe('AdminPaymentsComponent refunds', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        provideRouter([]),
        { provide: ActivatedRoute, useValue: { snapshot: { paramMap: convertToParamMap({ paymentId: 'payment-id' }), url: [] } } },
      ],
    });
  ...
  it('keeps the stock rule scoped to non-raffle orders and explains the conditional raffle rule', () => {
    ...
    refundButton()!.click();
    fixture.detectChanges();
    const warning = fixture.nativeElement.querySelector('.warning-note').textContent.replace(/\s+/g, ' ');
    ...
    expect(warning).toContain('Para pedidos que no son de rifa, el stock no se repone automáticamente');
  ...
  it('submits only the existing refund request and reloads backend detail without releasing numbers locally', () => {
    ...
    expect(request.request.body).toEqual({ reason: 'Solicitud del comprador', confirmation: 'REEMBOLSAR' });
  ...
  function load(status: AdminRefundOperation['status'] | null): void {
    const detail: AdminPaymentDetailResponse = {
      payment: {
        id: 'payment-id', providerPaymentId: 'provider-payment-id', orderId: 'order-id',
        providerStatus: 'approved', providerStatusDetail: null, processingStatus: 'APPLIED',
        transactionAmountInCents: 50000, currencyId: 'ARS', dateApproved: null,
        createdAt: '2026-09-01T12:00:00Z', reviewReason: null, reviewResolvedAt: null,
        reviewResolution: null, reviewResolvedByAdminId: null, reviewNote: null,
      },
      order: { id: 'order-id', status: 'PAID', totalInCents: 50000, createdAt: '2026-09-01T12:00:00Z', paidAt: null },
  ...
  function refund(status: AdminRefundOperation['status']): AdminRefundOperation {
    return { id: 'refund-id', paymentId: 'payment-id', orderId: 'order-id', amountInCents: 50000,
      status, providerRefundId: 'provider-refund-id', createdAt: '2026-09-01T12:00:00Z',
